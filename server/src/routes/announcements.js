...
router.get('/', checkJwt, async (req, res) => {
  try {
    ...
    const announcements = await Announcement.find(filter).sort({ createdAt: -1 }).limit(5).populate('createdBy', 'name').lean()
...
router.post('/', checkJwt, requireRole('admin', 'teacher'), async (req, res) => {
  try {
    ...
    const announcement = await Announcement.create({ ...req.body, createdBy: user._id })
